...
from utils.utils import str_to_bool, read_conf, get_dict_from_args
import os
import pickle
...
def get_label_by_name(speaker_name, speaker_label):
    return speaker_label[speaker_name]


# 根据标签获得人名，获得的人名也是小写的
def get_name_by_label(label_index, label_speaker):
    return label_speaker[label_index]


# 获得speaker_label 和label_spaeker
def get_speaker_label(MODE):
    if MODE == "TIMIT":
        speaker_label_file = 'prefile/speaker_label.plk'
        label_speaker_file = 'prefile/label_speaker.plk'
    elif MODE == "Librispeech":
        speaker_label_file = 'prefile/lib_speaker_label.plk'
        label_speaker_file = 'prefile/lib_label_speaker.plk'
    with open(speaker_label_file, 'rb') as f:
        speaker_label = pickle.load(f)
    with open(label_speaker_file, 'rb') as f:
        label_speaker = pickle.load(f)
    return speaker_label, label_speaker





def get_speaker_model(MODE):
    args = get_option(MODE)
    ...
    return pretrained_models['speaker']
